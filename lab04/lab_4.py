...
def gaussian_filter_low(sigma, size):
    x = np.linspace(-size / 2, size / 2, size)
    y = []
    for i in range(len(x)):
        temp = -x[i] ** 2 / (2 * sigma ** 2)
        y.append(math.exp(temp))
...
def butterworth_filter_low(D, size):
    x = np.linspace(-size / 2, size / 2, size)
    y = []
    for i in range(len(x)):
        y.append(1 / (1 + (x[i] / D) ** 4))
    s = sum(y)
    for i in range(len(y)):
        y[i] /= s
...
def main():
    ...
    '''for i in range(len(graphics)):
        axs[1][i].plot(t, filtfilt(b, a, graphics[i]))
        axs[1][i].grid(True)'''
    for i in range(len(graphics)):
        axs[1][i].plot(t, filtfilt(G, 1, graphics[i]))
        axs[1][i].grid(True)
        axs[1][1].set_title('Фильтр низкий частот: Гаусса')
    for i in range(len(graphics)):
        axs[2][i].plot(t, filtfilt(B, 1, graphics[i]))
        axs[2][i].grid(True)
